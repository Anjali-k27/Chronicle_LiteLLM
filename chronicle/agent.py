...
# ── Imports (Session 14.2 — LiteLLM Dynamic Model Router) ─────────
from model_router import (
    LiteLLMShim, VirtualKeyRegistry, LOGICAL_MODEL_MAP,
    route_chronicle_agent, AGENT_TIER_MAP,
)

# S14.2: shim and registry built once at import time (lifespan owns them in production)
_litellm_shim = LiteLLMShim(LOGICAL_MODEL_MAP)
_vk_registry  = VirtualKeyRegistry()
_chronicle_vk = _vk_registry.issue(
    owner="chronicle-agent",
    scope=list(LOGICAL_MODEL_MAP.keys()),
    budget_usd=100.0,
)
    ...
    async def pattern_node(state: ChronicleState) -> dict:
        ...
        with tracer.start_as_current_span("chronicle.pattern") as span:
            ...
            try:
                ...
                # S14.2: route through LiteLLM shim — picks cheapest model that clears the quality bar
                routed = await route_chronicle_agent(
                    agent       = "pattern",
                    prompt      = prompt,
                    shim        = _litellm_shim,
                    vk          = _chronicle_vk,
                    vk_registry = _vk_registry,
                    question    = state.get("question", ""),
                    use_semantic = False,
                )
                result = {"text": routed.content, "in_tok": routed.input_tokens, "out_tok": routed.output_tokens}
                span.set_attribute("routing_model",        routed.logical_model)
                span.set_attribute("routing_model_actual", routed.concrete_model)
                correlations = [
                    line.strip()
    ...
    async def brutality_node(state: ChronicleState) -> dict:
        ...
        with tracer.start_as_current_span("chronicle.brutality") as span:
            ...
            try:
                ...
                # S14.2: route through LiteLLM shim — semantic classifier may escalate/de-escalate tier
                routed = await route_chronicle_agent(
                    agent       = "brutality",
                    prompt      = prompt,
                    shim        = _litellm_shim,
