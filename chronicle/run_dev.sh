...
# Phoenix gets a much longer budget than the MCP servers: on a freshly
# created .venv (no bytecode cache yet), Phoenix's first-ever import
# pulls in pandas/sqlalchemy/pyarrow/scikit-learn and can easily take
# 60-90s before it binds :6006 — well past the old flat 30s, which
# failed here on a cold install even though Phoenix came up fine right
# after. Every later run is warm (cache hit) and comes up in seconds,
# so this only costs time on the very first run.
