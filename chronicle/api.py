...
# ── Imports (Session 14.1 — Semantic Cache) ───────────────────────
from semantic_cache import SemanticCache, GeminiEmbedder
...
async def lifespan(app: FastAPI):
    ...
    # ── S14.1: semantic cache — built once at startup, reused across requests ──
    # Embedder holds an LRU + counts API calls; the cache is stateful (hit/miss
    # counters, stored vectors), so both must be shared singletons, not
    # rebuilt per request.
    embedder        = GeminiEmbedder()
    app.state.cache = SemanticCache(embedder)
    log.info(f"Semantic cache ready — threshold={app.state.cache.threshold} ttl={app.state.cache.ttl_seconds}s")
...
app = FastAPI(
    ...
    description=(
        ...
        "Session 14.1 — Semantic Cache + FinOps Cost Model."
    ),
    version="14.2.0",
    lifespan=lifespan,
)
...
async def health():
    """
    Session 14.1 update: session → "14.1", semantic_cache + finops_cost_model
    capabilities enabled now that semantic_cache.py exists and is wired into
    /analyze/async.
    Session 14.2 update: session → "14.2", litellm_routing + virtual_key_registry
    + finops_per_agent capabilities enabled now that model_router.py is wired
    into the pattern/timeline/brutality/synthesis nodes.
    """
    ...
    return {
        ...
        "capabilities": [
            ...
            "semantic_cache",                     # S14.1
            "finops_cost_model",                  # S14.1
            "langgraph_node_spans",               # S13.1
            "ai_attribute_schema",                # S13.1
            "backgroundtask_context_prop",        # S13.1
            "litellm_routing",                    # S14.2
            "virtual_key_registry",                # S14.2
            "finops_per_agent",                    # S14.2
        ],
    }
...
async def _run_chronicle_analysis_and_cache(
    job_id:      str,
    ticket_id:   str,
    graph,
    ...
    """
    S14.1: wraps run_chronicle_analysis() so the semantic cache is populated
    from the REAL swarm's final_brief once the background job completes —
    not from a stand-in response. Scheduled via BackgroundTasks in place of
    run_chronicle_analysis() directly; forwards every argument unchanged.

    WHY not semantic_cache.semantic_cache_dep() inline on the miss path:
    that helper's miss branch calls swarm_invoke_stub(), which makes its own
    blocking Gemini call — fine for the semantic_cache.py standalone demo,
    but wrong here: it would (a) add several seconds to what must be a
    <100ms 202 ack, and (b) write a fake stand-in answer into the cache
    instead of Chronicle's actual synthesis. This wrapper runs after the
    real graph finishes, in the background, so neither problem applies.
    Introduced: Session 14.1. Permanent.
    """
    await run_chronicle_analysis(job_id, ticket_id, graph, request, analysis_id, otel_ctx)

    record = await read_job(job_id)
    if record is not None and record.status == JobStatus.COMPLETED and record.final_result:
        cache.put(ticket_text, record.final_result)
        log.info(f"Cache populated from completed job {job_id}")
...
async def analyze_async(
    ...
    # ── S14.1: cache check BEFORE idempotency and BEFORE BackgroundTask ────
    # A hit returns synchronously, before app.state.graph.astream() is ever
    # scheduled — zero graph compilation cost, zero MCP fetches, zero vLLM
    # calls for this request. A miss falls through to the existing 202 path
    # unchanged below. Calls cache.get() directly (not the semantic_cache_dep
    # convenience wrapper) so a miss never triggers an extra blocking Gemini
    # call on this hot path — see _run_chronicle_analysis_and_cache's
    # docstring for why.
    current_span    = otel_trace.get_current_span()
    cache_hit_entry  = app.state.cache.get(request.question)

    if cache_hit_entry is not None:
        cached_id = str(uuid.uuid4())
        current_span.set_attribute("cache_hit", True)
        current_span.set_attribute("cache_bypass_reason", "none")
        current_span.set_attribute("cache_usd_saved", app.state.cache.stats()["usd_saved"])

        log.info(f"Cache HIT for question={request.question[:60]!r} — swarm bypassed")

        # Write a COMPLETED job record so GET /analyze/jobs/{job_id} — and the
        # existing frontend poll loop — work unchanged for a cache hit. Without
        # this, poll_url points at a job_id that was never written, the first
        # poll 404s, and the UI sits there until it times out instead of
        # showing the instant cached result.
        await write_job(JobRecord(
            job_id=cached_id, ticket_id=cached_id, analysis_id=cached_id,
            question=request.question,
            status=JobStatus.COMPLETED,
            final_result=cache_hit_entry.consensus_response,
            processing_ms=0,
        ))
        ...
        return JobAcceptedResponse(
            ...
            poll_url=f"/analyze/jobs/{cached_id}",
            estimated_completion_seconds=0,
            retry_after_seconds=0,
        )

    current_span.set_attribute("cache_hit", False)
    current_span.set_attribute("cache_bypass_reason", "below_threshold")
    ...
    background_tasks.add_task(
        _run_chronicle_analysis_and_cache,
        job_id,
        job_id,
        app.state.graph,
        request,
        ...
        app.state.cache,   # S14.1: populate cache from the real swarm output
        request.question,
    )
