...
SWARM_SLOS = [
    ...
    SLO(
        name="served_hit_rate",
        sli_description="rolling served cache hit rate over last 300 tickets",
        target="> 30% sustained after warm-up",
        window_seconds=300,
        span_attributes=["cache_hit"],
        breach_action="investigate: prompt drift · embedding-model swap · threshold miscalibration",
    ),
    SLO(
        name="frontier_tier_fraction",
        sli_description="fraction of LLM calls routed to frontier-model",
        target="< 30% of total calls (brutality + synthesis only)",
        window_seconds=300,
        span_attributes=["routing_model"],
        breach_action="investigate: tier assignment drift · semantic classifier regressing to frontier",
    ),
]
...
class PhoenixTraceRecord(BaseModel):
    ...
    cache_hit:            Optional[bool] = None  # S14.1: None = not a cache-checked trace
    routing_model:        Optional[str] = None   # S14.2: logical tier requested (None = not routed)
    routing_model_actual: Optional[str] = None   # S14.2: concrete model that actually ran
...
class MonitoringDaemon:
    ...
    async def tick(self) -> None:
        ...
        Seven checks in order:
          ...
          5. Cache hit rate collapse — only once warmed up (>= 20 labelled traces)
          6. Three-strikes Judge — sampled judge scores
          7. Frontier-tier fraction spike — only once warmed up (>= 10 routed spans)
        ...
        # ── 5. Cache hit rate collapse ────────────────────────────────────────
        cache_hit_traces = [t for t in traces if t.cache_hit is True]
        cache_total      = len([t for t in traces if t.cache_hit is not None])
        if cache_total >= 20:   # only evaluate after enough labelled traffic
            hit_rate = len(cache_hit_traces) / cache_total
            if hit_rate < 0.20:   # sustained collapse below 20% (target: 30%)
                self._page(
                    "cache_hit_rate_collapsed", "prompt", traces[-1],
                    {
                        "observed_hit_rate": round(hit_rate, 3),
                        "target_hit_rate":   0.30,
                        "labelled_traces":   cache_total,
                        "possible_causes":   "prompt drift · embedding swap · threshold miscal",
                    },
                )

        # ── 6. Judge quality — sampled traces only ───────────────────────────
        ...
        # ── 7. Frontier-tier fraction spike ──────────────────────────────────
        routed_spans   = [t for t in traces if t.routing_model is not None]
        frontier_spans = [t for t in routed_spans if 'pro' in (t.routing_model_actual or '')]
        if len(routed_spans) >= 10:
            frontier_fraction = len(frontier_spans) / len(routed_spans)
            if frontier_fraction > 0.40:   # > 40% → something routed wrong
                self._page(
                    "frontier_tier_fraction_spike", "prompt", traces[-1],
                    {
                        "frontier_fraction":  round(frontier_fraction, 3),
                        "target_fraction":    0.30,
                        "labelled_spans":     len(routed_spans),
                        "likely_cause":       "semantic classifier confidence floor too low · AGENT_TIER_MAP drift",
                    },
                )
